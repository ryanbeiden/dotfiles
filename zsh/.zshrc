...
# Install dependencies without changing lock files (unless necessary)
alias install-dependencies="npm ci && composer install --no-dev --prefer-dist --classmap-authoritative"
...
artifacts() {
  if [[ "$PWD" == "$HOME/workspace/projects/artifacts/php" ]]; then
    php bin/console "$@"
  else
    print -P "%F{yellow}Wrong directory... Changing directory to $HOME/workspace/projects/artifacts/php"
    cd "$HOME/workspace/projects/artifacts/php"
    php bin/console "$@"
  fi
}
